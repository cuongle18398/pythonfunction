import pyodbc
import pandas as pd
import time
import openpyxl

def uploadTarget(server, database, username, password):
    try:
        excel_file_path = 'Input/Target_Input.xlsx'
        table_name = 'Monthly_Target_Import_Temp'
        #Tạo connection và đọc file excel
        print("Đang kết nối cơ sở dữ liệu.")
        time.sleep(1)
        connection_string = f'DRIVER={{SQL Server}};SERVER={server};DATABASE={database};UID={username};PWD={password}'
        connection = pyodbc.connect(connection_string)
        excel_data = pd.read_excel(excel_file_path, skiprows=3)
        insert_query = f'INSERT INTO {table_name} ({", ".join(excel_data.columns)})'
        cursor = connection.cursor()
        time.sleep(1)
        print("Kết nối thành công. Dữ liệu đang được import!")
        time.sleep(1)
        for index, row in excel_data.iterrows():
            values = ""
            for i in range(0,39):
                values = f"{values},'{row.iloc[i]}'"
            values = values.lstrip(',')
            query = f"{insert_query} VALUES ({values})"
            cursor.execute(query)
            values = ""
            print(f'Line {row.iloc[3]} input !')
        print("Commit database.. Cập nhật vào hệ thống")
        cursor.execute('EXEC sp_Monthly_Target_Python_Import')
        print("Commit database..")
        connection.commit()
        cursor.close()
        connection.close()
        time.sleep(1)
        print("Dữ liệu đã được cập nhật thành công.")
    except Exception as e:
        print(f"An error uploadTarget function occurred: {str(e)}")


def deleteTarget(server, database, username, password, month):
    try:
        print("Đang truy xuất cơ sở dữ liệu.")
        time.sleep(2)
        connection_string = f'DRIVER={{SQL Server}};SERVER={server};DATABASE={database};UID={username};PWD={password}'
        connection = pyodbc.connect(connection_string)
        query = f'EXEC [sp_Monthly_Target_Remove] @month = {month}'
        cursor = connection.cursor()
        cursor.execute(query)
        connection.commit()
        cursor.close()
        connection.close()
        time.sleep(1)
        print("Dữ liệu Target được xóa thành công.")
    except Exception as e:
        print(f"An error deleteTarget function occurred: {str(e)}")