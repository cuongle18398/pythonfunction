...
def statusUpdate(server, database, username, password):
    try:
        excel_file_path = 'Input/Status_Input.xlsx'
        table_name = 'ERP_StatusItem_Temp'
        ...
        cursor.execute('EXEC sp_ERP_StatusItem_Update')
        connection.commit()
        cursor.close()
        connection.close()
        time.sleep(1)
...
def item_choice_fn(choice):
    print(f"Đang xử lý yêu cầu. Đảm bảo cập nhật đúng thông tin {choice} trong file Item_Input.")
    excel_file_path = 'Input/Item_Input.xlsx'
    table_name = 'ERP_StatusItem_Temp'